...
class Epub():
    ...
    def set_file_path(self, file_path):
        ...
        self.cache_path += os.path.basename(file_path) + "EXTRACTED/"
        if not os.path.exists(self.cache_path):
            os.mkdir(self.cache_path)
    ...
    def prepare(self):
        container = ElementTree.iterparse(
            self.cache_path+"META-INF/container.xml")

        for event, element in container:
        ...
        for event, element in ElementTree.iterparse(self.opf_path):
            ...
            if re.search('^item$|}item$', element.tag):
                """
                we get the manifest : all files unordered,
                their id, their href, their mimetype
                yay, tuple time ! <(°°<)  ^(°°)^  (>°°)>
                """
                manifest.append((element.attrib['id'],
                                 element.attrib['href'],
        ...
        for count, i in enumerate(spine):
            for item in manifest:
                if item[2] == "application/xhtml+xml":
                    if item[0] == i:
                        self.chapters[int(count)] = Chapter(item[0],
                                                            os.path.dirname(self.opf_path)+"/"+item[1])
